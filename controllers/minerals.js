...
exports.minerals_view_one_Page = async function(req, res) {
   ...
   // Handle building the view for creating a mineral.
// No body, no in path parameter, no query.
// Does not need to be async
exports.minerals_create_Page = function(req, res) {
    console.log("create view")
    try{
    res.render('mineralscreate', { title: 'Minerals Create'});
    }
    catch(err){
    res.status(500)
    res.send(`{'error': '${err}'}`);
   ...
   // Handle building the view for updating a minerals.
// query provides the id
exports.minerals_update_Page = async function(req, res) {
    console.log("update view for item "+req.query.id)
    try{
    let result = await minerals.findById(req.query.id)
    res.render('mineralsupdate', { title: 'Minerals Update', toShow: result });
    }
    catch(err){
    res.status(500)
    res.send(`{'error': '${err}'}`);
...
exports.minerals_delete_Page = async function(req, res) {
    console.log("Delete view for id " + req.query.id)
    try{
    result = await minerals.findById(req.query.id)
    res.render('mineralsdelete', { title: 'Minerals Delete', toShow: 
   result });
    }
    catch(err){
    res.status(500)
